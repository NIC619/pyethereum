...
from ethereum.utils import sha3, encode_hex, normalize_address
...
def mk_storage_proof_wrapper(db, storage_root, storage_key):
    """Generate a merkle proof wrapper for a given storage key of a given account
    """
    proof_wrapper = {}
    proof_wrapper["storage_root"] = storage_root
    proof_wrapper["storage_key"] = storage_key
    proof_wrapper["storage_value"] = trie._get(db, storage_root, trie.encode_bin(sha3(storage_key)))
    proof_wrapper["merkle_proof"] = get_merkle_proof(db, storage_root, storage_key)
...
def mk_pending_tx_bundle(cur_state, tx, latest_blk_header, latest_state):
    ...
    accessed_storage_key_list_proof = []
    for acct_stgkey in tx.accessible_storage_key_list:
        acct = acct_stgkey[:20]
        stgkey = acct_stgkey[20:]
        o = mk_storage_proof_wrapper(cur_state, latest_state.get_and_cache_account(acct).storage, stgkey)
        accessed_storage_key_list_proof.append({acct: o})
    tx_bundle["accessed_storage_key_list_proof"] = accessed_storage_key_list_proof
    return tx_bundle
...
def mk_confirmed_tx_bundle(cur_state, tx, prev_blk_header, latest_blk_header, prev_state):
    ...
    for acct_stgkey in tx.accessible_storage_key_list:
        acct = acct_stgkey[:20]
        stgkey = acct_stgkey[20:]
        o = mk_storage_proof_wrapper(cur_state.trie.db, prev_state.get_and_cache_account(acct).storage, stgkey)
        accessed_storage_key_list_proof.append({acct: o})
    tx_bundle["accessed_storage_key_list_proof"] = accessed_storage_key_list_proof
    updated_acct_list = []
    ...
    updated_storage_key_list = []
    for acct_stgkey in tx.accessible_storage_key_list:
        acct = acct_stgkey[:20]
        stgkey = acct_stgkey[20:]
        o = mk_storage_proof_wrapper(cur_state.trie.db, cur_state.get_and_cache_account(acct).storage, stgkey)
        updated_storage_key_list.append({acct: o})
    tx_bundle["updated_storage_key_list"] = updated_storage_key_list
    return tx_bundle

def verify_confirmed_tx_bundle(env, state_root, coinbase, tx_bundle):
    ...
    for acct_proof_wrapper in tx_bundle["write_list_proof"]:
        for acct, wrapper in acct_proof_wrapper.items():
            ...
            store_merkle_branch_nodes(ephem_state.trie.db, wrapper["merkle_proof"])
    # And storage key too
    for stgkey_proof_wrapper in tx_bundle["accessed_storage_key_list_proof"]:
        for acct, wrapper in stgkey_proof_wrapper.items():
            assert wrapper["storage_root"] == ephem_state.get_and_cache_account(acct).storage
            assert verify_merkle_proof(wrapper["merkle_proof"], wrapper["storage_root"], sha3(wrapper["storage_key"]), wrapper["storage_value"])
            # Store the storage data after verifying the proof
            store_merkle_branch_nodes(ephem_state.trie.db, wrapper["merkle_proof"])
